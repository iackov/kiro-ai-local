...
$body = "message=Detect system optimization opportunities&auto_execute=true"
try {
    $resp = Invoke-RestMethod -Uri "$baseUrl/api/autonomous" -Method Post -Body $body -ContentType "application/x-www-form-urlencoded" -TimeoutSec 30
    if ($resp.task_result) {
        Write-Host "  Task Status: $($resp.task_result.status)" -ForegroundColor Cyan
        ...
        Write-Host "  Task Status: query (not executed)" -ForegroundColor Yellow
        Write-Host "  Success Rate: N/A" -ForegroundColor Yellow
    }
} catch {
    Write-Host "  Task Status: failed" -ForegroundColor Red
...
$body = "message=Add Redis cache service to docker-compose&auto_execute=true"
try {
    ...
    if ($resp.task_result) {
        ...
        Write-Host "  Task Status: query (not executed)" -ForegroundColor Yellow
        Write-Host "  Total Steps: N/A" -ForegroundColor Yellow
    }
} catch {
    Write-Host "  Task Status: failed" -ForegroundColor Red
    Write-Host "  Total Steps: 0" -ForegroundColor White
    Write-Host "  Successful: 0" -ForegroundColor Red
...
try {
    ...
    if ($resp.task_result) {
        ...
        Write-Host "  Context Preserved: N/A" -ForegroundColor Yellow
        Write-Host "  Steps: N/A" -ForegroundColor Yellow
    }
} catch {
    Write-Host "  Context Preserved: No" -ForegroundColor Red
    Write-Host "  Steps: 0/0" -ForegroundColor Red
}
Write-Host ""
