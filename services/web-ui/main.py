...
app = FastAPI(title="AI Combiner Stack - Web UI")
...
async def get_status():
    ...
    async with httpx.AsyncClient(timeout=5.0) as client:
        # Check RAG API
        try:
            resp = await client.get(f"{SERVICES['rag']}/health")
        ...
        # Check Arch Engine
        try:
            resp = await client.get(f"{SERVICES['arch']}/health")
...
async def get_rag_stats():
    ...
    async with httpx.AsyncClient(timeout=10.0) as client:
        try:
            resp = await client.get(f"{SERVICES['rag']}/inspect")
            return resp.json()
        except Exception as e:
            return {"error": str(e)}

@app.post("/api/rag/query")
async def rag_query(query: str = Form(...), top_k: int = Form(5)):
    """Query RAG database"""
...
async def get_arch_history():
    ...
    async with httpx.AsyncClient(timeout=5.0) as client:
        try:
            resp = await client.get(f"{SERVICES['arch']}/arch/history")
            return resp.json()
        except Exception as e:
            return {"error": str(e)}
...
async def combined_query(query: str = Form(...), top_k: int = Form(3)):
    ...
    try:
        ...
        if any(word in query.lower() for word in ["add", "service", "docker", "compose", "architecture"]):
            async with httpx.AsyncClient(timeout=10.0) as client:
                arch_resp = await client.get(f"{SERVICES['arch']}/arch/history")
